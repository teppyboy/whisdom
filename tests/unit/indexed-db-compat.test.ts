...
const LEGACY_SETTINGS_V2 = Object.fromEntries(
  Object.entries(LEGACY_SETTINGS).filter(([key]) => key !== "experimentalVad")
)
const CANONICAL_ROW = {
  schemaVersion: 2,
...
async function createV1(
  settings: unknown = LEGACY_SETTINGS,
  rows: unknown[] = [LEGACY_DOCUMENT]
) {
...
async function createV2(overrides: V2Overrides = {}) {
  const db = await openDB("whisdom", 2, {
    upgrade(database) {
      ...
      if (!overrides.brokenIndex)
        transcripts.createIndex("by-updatedAt", "updatedAt")
      const quarantine = database.createObjectStore("migrationQuarantine", {
      ...
      const conflicts = database.createObjectStore("conflictCandidates", {
        keyPath: "candidateId",
      })
      conflicts.createIndex("by-receivedAt", "receivedAt")
      conflicts.createIndex("by-transcriptId", "transcriptId")
      const metadata = database.createObjectStore("syncMetadata", {
      ...
      const pending = database.createObjectStore("pendingOperations", {
        keyPath: ["accountKey", "transcriptId"],
      })
  ...
  if (overrides.settings !== undefined)
    await db.put("settings", overrides.settings, "settings")
  if (overrides.omitStore !== "meta")
    await db.put("meta", overrides.deviceId ?? DEVICE_ID, "deviceId")
  for (const row of overrides.rows ?? [CANONICAL_ROW])
    await db.put("transcripts", row)
  db.close()
}
...
describe("MIG-01 compatibility adapter", () => {
  it("preserves exact legacy behavior on fresh and v1 databases", async () => {
    ...
    const older: TranscriptDocument = {
      ...LEGACY_DOCUMENT,
      id: "tr_older",
    ...
    expect((await listTranscripts()).map((row) => row.id)).toEqual([
      "tr_legacy_001",
      "tr_older",
    ])
  ...
  it("increments revision on v2 rename and preserves lineage", async () => {
    await createV2()
    const renamed = await renameTranscript(
      "tr_sample_001",
      "Renamed transcript"
    )
    expect(renamed).toMatchObject({
    ...
    expect(row.transcript).toMatchObject({
      createdAt: 1785283200000,
      sourceName: "sample.wav",
      segments: CANONICAL_ROW.transcript.segments,
    })
    expect((row.transcript as { title: string }).title).toBe(
      "Renamed transcript"
    )
  })
  ...
  it("creates a v2 live envelope at revision 0 without pending operations", async () => {
    ...
    const created = (await readTranscriptRows()).find(
      (row) => row.transcriptId === "tr_new_001"
    )
    ...
    expect(created!.transcript).toMatchObject({
      ...
      text: "Legacy text",
      segments: [
        { id: "seg-legacy", startMs: 0, endMs: 1250, text: "Legacy text" },
      ],
    })
  ...
  it("writes a canonical tombstone on v2 delete and omits it from the list", async () => {
    ...
    const tombstone = (await readTranscriptRows()).find(
      (row) => row.transcriptId === "tr_sample_001"
    )!
  ...
  it("restores an observed tombstone through explicit lineage", async () => {
    ...
    expect((await readTranscriptRows())[0]).toMatchObject({
      ...
      deletedAt: null,
      deletionId: null,
      restoredFromDeletionId: deletionId,
    })
  })
  ...
  it("clears v2 rows as tombstones and leaves existing tombstones unchanged", async () => {
    ...
    const before = (await readTranscriptRows()).find(
      (row) => row.transcriptId === "tr_new_001"
    )

    await clearTranscripts()
    const rows = await readTranscriptRows()
    expect(rows.find((row) => row.transcriptId === "tr_new_001")).toEqual(
      before
    )
    expect(
      rows.find((row) => row.transcriptId === "tr_sample_001")
    ).toMatchObject({ revision: 4, transcript: null })
    expect(await listTranscripts()).toEqual([])
  })
  ...
  it.each([
    ...
    [
      "malformed row",
      { rows: [{ ...CANONICAL_ROW, revision: -1 }] },
      "storage.malformed-v2",
    ],
    [
      "exhausted revision",
      { rows: [{ ...CANONICAL_ROW, revision: Number.MAX_SAFE_INTEGER }] },
      "storage.revision-exhausted",
    ],
  ] as const)(
    "refuses the complete mutation for %s",
    async (_label, overrides, code) => {
      await createV2(overrides)
      await expectRefusal(
        () => renameTranscript("tr_sample_001", "Attempt"),
        code
      )
    }
  )
    ...
    await expectRefusal(
      () =>
        saveTranscript({
          ...LEGACY_DOCUMENT,
          id: "tr_new_001",
          segments: [{ id: "seg", start: 0, end: 604801, text: "Legacy text" }],
        }),
  ...
  it.each([3, 17])(
    "closes and refuses every public call on unsupported version %s",
    async (version) => {
  ...
  it("projects both settings key sets and preserves stored explicitModelId", async () => {
    await createV2({ settings: LEGACY_SETTINGS_V2 })
  ...
  it.each([
    ...
    [
      "lone surrogate explicit model",
      { ...LEGACY_SETTINGS, explicitModelId: "\ud800" },
    ],
  ])("fails closed for %s settings", async (_label, settings) => {
    await createV2({ settings })
    await expect(loadSettings()).rejects.toBeInstanceOf(
      StorageCompatibilityError
    )
  })
})
...
describe("storage compatibility API bootstrap", () => {
  it("installs exactly the seven frozen adapter references", async () => {
    ...
    try {
      const { STORAGE_COMPATIBILITY_API } =
        await import("@/features/storage/compatibility-api")
      ...
      expect(window.__WHISDOM_STORAGE_COMPATIBILITY__).toBe(
        STORAGE_COMPATIBILITY_API
      )
    } finally {
      if (original)
        ...
        Reflect.deleteProperty(globalThis, "__WHISDOM_STORAGE_COMPATIBILITY__")
    }
  })
})
