...
- The token lives in React memory at `src/App.tsx:622`; expiry is not tracked. An unsupported background token reacquisition is attempted only immediately before a local-mode upload at `src/App.tsx:1149-1169`.
...
| Synced | Exact local candidate is verified by known Drive file ID, wins the stabilized observed candidate set, and has no pending operation; this describes verified observed state, never a global Drive snapshot |
...
4. Undo/Restore may act only on a tombstone the user action observed. It clears deletion, sets `restoredFromDeletionId` to that exact tombstone’s `deletionId`, writes a greater revision, and enqueues the coalesced candidate publication. No background or stale live mutation may create, copy, or guess this field. Undo remains available after toast expiry through recovery only when product exposes a deleted filter.
5. Reconciliation evaluates restore eligibility only against the regular-order `dominantTombstone` from the complete validated set. A live record that names another deletion ID is ineligible; a non-dominant tombstone with another deletion ID does not independently override an eligible restore.
...
| Connected | Token enters the 60-second freshness margin or expires | Needs reconnect; reconciliation paused |
| Needs reconnect | Explicit Connect, Reconnect, or Sync now gesture | Opening |
...
Google Identity Services supplies short-lived access tokens only; its browser token model supplies no refresh token. Each auth attempt has a unique attempt ID. The success callback and GIS `error_callback`, when available, may settle only the current ID. A bounded watchdog invalidates the ID on timeout, moves to Signed out or Needs reconnect as appropriate, and ignores every late callback. Popup dismissal/blocking and interaction-required responses must settle visibly. Granular consent may omit requested scopes, so the client must verify the granted scopes before UserInfo or Drive calls and move to localized Needs reconnect with a scope-specific recovery action when required scopes are absent.

Access tokens and `expires_in`-derived expiry remain memory-only. Apply a 60-second freshness margin. Entering that margin or reaching expiry pauses reconciliation and moves to localized Needs reconnect; the app never calls `requestAccessToken` from a timer, focus handler, reconcile loop, or other background path. Token reacquisition may begin only from an explicit Connect, Reconnect, or Sync now user gesture. During that gesture, `prompt: ''` may reduce repeated prompts, but it remains a user-initiated token request and must never be described or implemented as background token reacquisition. After reload, stale non-secret identity metadata may be displayed as stale, but Drive access requires the same explicit gesture. Never persist an access token, authorization header, refresh token, or ID token.

Sign out clears in-memory credentials and pauses sync; local transcripts remain. Revocation must call `google.accounts.oauth2.revoke`; do not send a direct request to a revocation endpoint. Revoke success is shown only after the GIS revoke callback confirms success. Confirmation copy must state that revoking disconnects every Google authorization granted to the same Cloud project, not only Drive sync. On callback failure/timeout, clear local credential/session state but report that revocation is unconfirmed and link to Google Account permissions at `https://myaccount.google.com/permissions`. If no usable token exists, do not claim or simulate revocation: clear local state, state that revocation is unconfirmed, and provide the same link. Revocation does not erase existing backup files. A separate “Delete Drive backup” operation is out of scope.
...
- Avatar rendering is fallback-first. Validate the picture string as a Unicode scalar sequence before URL construction or display. Then validate the initial URL before any request: scheme must be HTTPS, hostname must be in the explicit allowlist (initially exactly `lh3.googleusercontent.com`), credentials must be absent, and the port must be empty or the standard HTTPS port `443`. Fetch with `redirect: "error"`, a five-second timeout, and a 1 MiB streamed byte cap; require an `image/*` MIME type, then render a revocable Blob URL. Any redirect is rejected because browser Fetch cannot expose or count cross-origin redirect hops. Never inspect `response.url` as a redirect-chain security control, and never place the remote picture URL directly in `img src`. Google does not guarantee direct picture URL, host, or CORS stability; a validated fetch may therefore fail to initials or the generic account glyph without blocking identity activation or sync. Redirect rejection, CORS prevention, timeout, invalid MIME, oversize response, malformed Unicode, or any validation failure uses that fallback. Do not broaden the approved host allowlist to make an avatar load.
...
Static-host CSP must enumerate narrow endpoints: `script-src https://accounts.google.com/gsi/client`; `frame-src https://accounts.google.com`; `connect-src https://accounts.google.com https://openidconnect.googleapis.com https://www.googleapis.com https://lh3.googleusercontent.com`; and `img-src 'self' blob: data:`. Keep the project’s required self directives. Do not add wildcard hosts. Calls are limited to GIS under `https://accounts.google.com/gsi/`, UserInfo `https://openidconnect.googleapis.com/v1/userinfo`, Drive `https://www.googleapis.com/drive/v3/` and `https://www.googleapis.com/upload/drive/v3/`, and bounded avatar fetch on the exact allowlisted origin `https://lh3.googleusercontent.com/`. Revocation uses the loaded GIS client API.

### 15.4 Remote record

Each remote candidate is an immutable schema-2 envelope in `appDataFolder` and uses MIME type `application/vnd.whisdom.transcript+json`. Validate `transcriptId` as a Unicode scalar sequence before UTF-8 encoding or `remoteKey` hashing. Then let `remoteKey = base64url(SHA-256(UTF-8(transcriptId)))`, using the RFC 4648 URL-safe alphabet without padding. It is exactly 43 ASCII characters and must be recomputed from the parsed JSON `transcriptId` before accepting a file. A malformed-scalar `transcriptId` is rejected and never derives a key.

After the complete envelope passes the existing Unicode-scalar, exact-schema, canonical-value, and bound validation, serialize the complete envelope—not the accepted-payload projection—with RFC 8785. Compute `candidateHash = base64url(SHA-256(canonical full-envelope UTF-8 bytes))` using the same pinned unpadded RFC 4648 form; it is exactly 43 ASCII characters. `candidateHash` is distinct from `remoteKey`, quarantine `bodyByteHash`, and the existing lowercase-hex accepted-payload hash used by conflict ordering. The exact Drive filename is `whisdom-transcript-<remoteKey>-<candidateHash>.json`. Exact private `appProperties` are `whisdomTranscriptKey=<remoteKey>`, `whisdomSchemaVersion=2`, and `whisdomCandidateHash=<candidateHash>`. Each key is 20 ASCII bytes; key plus value is respectively 63, 21, and 63 bytes, so every property fits Drive's 124-byte key-plus-value constraint under the pinned 43-byte key/hash lengths. Three private properties remain within Drive's property-count limit. Raw `transcriptId` must never appear in a filename or appProperty; it remains inside JSON.
...
**Unicode scalar precondition.** Every string in a schema-2 envelope or payload, every UserInfo/avatar-URL-derived string, and every remote or local legacy string must be a valid Unicode scalar sequence before canonical whitespace handling, URL/display derivation, scalar or UTF-8 byte counting, persistence into a canonical store, `remoteKey` derivation, RFC 8785 serialization, `candidateHash`, or accepted-payload hashing. Implement validation over JavaScript UTF-16 code units: each high surrogate `U+D800..U+DBFF` must be immediately followed by one low surrogate `U+DC00..U+DFFF`; every low surrogate must be immediately preceded by one high surrogate. Reject lone, reversed, or otherwise unpaired surrogates. A valid surrogate pair counts as one scalar value. Never repair malformed input or replace it with `U+FFFD`. Schema-2 remote input rejects any malformed-scalar string. Malformed local legacy input follows the exact migration-quarantine disposition below and is never canonicalized, hashed, or uploaded. RFC 8785 input may be formed only after this validation; malformed Unicode is a hard parser/hash error. The selected RFC 8785 library or package must not be trusted to reject lone surrogates: the explicit scalar validator must run first, and tests must prove the canonicalizer is never invoked for malformed scalar input. This precondition does not apply to `bodyByteHash`, which hashes raw response bytes before UTF-8 decoding.
...
`deletionId` is stable for that tombstone across retries, duplicate files, reconcile, and repeated publication attempts. It is generated only for a new delete event, never regenerated for the same tombstone envelope. A valid live mutation descended from an explicit restore preserves its `restoredFromDeletionId`; unrelated or stale live state cannot synthesize or change it. A later delete replaces that live lineage with a fresh `deletionId` and null `restoredFromDeletionId`.

Remote Drive file IDs are implementation metadata only. Store them locally only for exact-file readback and permitted cleanup; never expose them in copy, logs, URLs, exports, or analytics. Drive `version`, when returned, may be stored as informational change metadata only. It never establishes causality, candidate precedence, or compare-and-swap safety. `modifiedTime` has the same non-causal status.

Schema-2 sync uses this immutable append-only candidate-set protocol. Drive `files.list` pagination is non-transactional and is not a snapshot; no completed pass proves that another candidate is absent:

1. List every candidate for the logical `remoteKey` with Drive `files.list`, `spaces=appDataFolder`, `trashed=false`, the `whisdomTranscriptKey=<remoteKey>` private-property predicate, explicit fields, and complete `nextPageToken` pagination. Include file `id,name,mimeType,appProperties,size,modifiedTime,version`; only `id`, validated content, name, MIME type, properties, and size affect protocol decisions. An invalid or rejected page token invalidates and restarts that complete pass. `incompleteSearch=true` invalidates the pass and schedules retry; its partial results cannot establish stability or authorize cleanup.
2. Download every unknown candidate under the streamed 25 MiB cap in Section 15.8. Strictly parse it, then recompute and require exact `remoteKey`, full-envelope `candidateHash`, existing accepted-payload hash, filename, MIME type, and all three appProperties. Quarantine malformed or mismatched data under Section 15.8. Persist every valid distinct candidate durably before conflict application.
3. Resolve the complete validated observed set with the pure set function in Section 15.8. Never use Drive file ID, `modifiedTime`, or Drive `version` as causal or content order. File ID selects the representative only among byte-identical physical copies with the same validated `candidateHash`.
4. Preserve every distinct losing candidate durably. Surface concurrent losing live content as conflict/history rather than deleting or collapsing it.
5. Coalesce pending operations by account key plus transcript ID only while publication is `unbound`: replace `desiredCandidateHash` and `desiredEnvelopeJson` with the latest unsynced eligible local candidate. Binding and retry semantics are defined below.
6. After read/resolve, publish the desired local candidate only when its exact `candidateHash` is not present in the accumulated validated observed set and it remains eligible; this is an idempotence optimization, not proof of global absence. Before its first create attempt, call Drive `files.generateIds` with `count=1` and `space=appDataFolder`. In one IndexedDB transaction, bind that generated ID and freeze the attempted candidate and all request-verification data before any `files.create`: generated Drive file ID, exact canonical full-envelope UTF-8 bytes/string, candidateHash, accepted-payload hash, exact filename, MIME type, three appProperties, and attempt state. Supply the generated ID as metadata `id` in the immutable multipart `files.create` body.
7. Every timeout, network failure, 408, 429, or 5xx retry for a bound attempt reuses the same generated ID and byte-for-byte request body. A 409 triggers `files.get` metadata plus media for that same ID and verifies exact body, full-envelope hash, accepted-payload hash, filename, MIME type, and appProperties; an exact match is success. After an ambiguous create, a transient 404 or unreadable same-ID response retries boundedly with that same ID/body. Exhausting the existing retry budget becomes Needs attention. Never allocate a second ID or reserialize/rebind content for the same bound attempt.
8. Read back the exact created file by the generated ID and perform the same exact verification. Every reconcile unions validated candidates from its initial pass, exact-ID readbacks, and all post-create passes. After publication, run at most four complete stabilization passes. Stability requires two consecutive complete passes with identical validated `(fileId,candidateHash)` sets and no local create between those passes. If stability is not reached by the fourth pass, retain the pending operation, schedule a bounded-backoff rerun, perform no cleanup, and do not transition to Synced. A later newly observed candidate reopens reconciliation.
9. Cleanup uses permanent Drive `files.delete`; `appDataFolder` has no trash. Absence from any list pass never proves deletion and never authorizes cleanup. Cleanup may target only positively identified, previously downloaded and validated file IDs satisfying retention rules. It may delete duplicate physical files with identical validated `candidateHash` after retaining one verified copy, or an older live candidate only when a strictly higher-revision live candidate from the same `deviceId` is verified; same-revision divergence must remain. Never delete a tombstone candidate or a distinct cross-device losing candidate without a future replica-watermark protocol. Limit cleanup to 20 files per reconcile and retry safely.

This cleanup bounds ordinary same-device revision growth while preserving concurrent branches. Tombstone and conflict retention remain a documented storage-growth tradeoff. Local-first durability and the deletion/restore set-resolution rules remain authoritative.

Each `pendingOperations` record retains its existing account key, transcript ID, retry count, next-attempt time, and error fields, and defines these exact semantic fields:

| Field | Semantics |
| --- | --- |
| `generatedFileId` | Null only while `publicationState` is `unbound`; generated Drive ID frozen for the bound create attempt |
| `attemptedCandidateHash` | Null only while `unbound`; frozen full-envelope hash for the bound create attempt |
| `attemptedEnvelopeJson` | Null only while `unbound`; exact canonical full-envelope UTF-8 string used as the multipart media body, never regenerated after binding |
| `desiredCandidateHash` | Latest eligible local candidate requested for eventual publication |
| `desiredEnvelopeJson` | Exact canonical full-envelope UTF-8 string corresponding to `desiredCandidateHash` |
| `publicationState` | Exact enum `unbound`, `bound`, `creating`, `verifying`, or `needs-attention` |

`unbound` permits desired-candidate coalescing and has no generated or attempted identity. `bound` means ID, attempted hash/body, accepted-payload hash, filename, MIME, appProperties, and attempt metadata are durable but no create is in flight. Before sending the request, transactionally change state to `creating`; because that state may represent a request whose outcome is unknown, recovery treats it as ambiguous. `verifying` means same-ID metadata/media verification or stabilization is active. `needs-attention` is terminal for automatic retries until explicit user recovery. After the first create attempt, `generatedFileId`, `attemptedCandidateHash`, `attemptedEnvelopeJson`, and the bound verification metadata are immutable.

A new local revision always updates `desiredCandidateHash` and `desiredEnvelopeJson`. Before binding, it may replace the candidate to be attempted. After binding or the first create attempt, it cannot change the frozen attempted candidate or resolve ambiguity by rebinding. Once the attempted candidate reaches verified success, remove that completed attempt; if desired differs from attempted, create a new `unbound` pending publication for the latest desired candidate. Corrupt or mismatching content under `generatedFileId` becomes `needs-attention` and blocks automatic rebinding or publication of the desired candidate through that operation.

Legacy Drive migration recognizes existing Whisdom `TranscriptDocument` files named `{id}.json` only through a bounded legacy discovery pass; raw names are never copied into new metadata. Cap the pass by normal Drive pagination, the 25 MiB body limit, and the exact remote legacy contract in Section 15.4, including the 16 KiB remote-ID intake cap. Every downloaded legacy body uses the Section 15.8 raw-byte fingerprint disposition before decode or parse. Remote legacy timing and segment IDs are strict: no local timing or ID repair applies. Validate scalar sequences before every bound, key derivation, serialization, `candidateHash`, or accepted-payload hash operation. Only a successfully canonicalized record may be imported locally and published as an immutable schema-2 candidate. IDs from 1 through 512 UTF-8 bytes may derive `remoteKey`; IDs from 513 bytes through 16 KiB fail the canonical target bound and therefore remain bounded remote quarantine metadata rather than a partial local transcript. Empty/all-`CANONICAL_WS` IDs, IDs above 16 KiB, malformed Unicode, and every other invalid body receive the same metadata-only disposition with `bodyByteHash` scope or bounded fingerprint-unavailable reason. Verify every new candidate body, recomputed keys/hashes, filename, MIME type, and appProperties before permanently deleting the old file. Cleanup remains bounded to 20 old files per reconcile and never runs before verified candidate persistence. Invalid old remote files are not cleaned up by migration. Never truncate an ID. Separately, locally migrated scalar-valid IDs over 512 bytes remain recoverable in `migrationQuarantine` as Needs attention until explicit safe-ID repair creates a canonical record.
...
2. Save to IndexedDB.
3. Increment revision and enqueue/coalesce pending candidate publication in the same transaction.
...
1. Validate account, every required granted scope, and the 60-second token freshness margin. When stale, pause and require an explicit Connect, Reconnect, or Sync now gesture.
2. Start a complete initial `files.list` pass before any pending create. Pagination is not a snapshot and no pass proves absence. Restart the pass on an invalid/rejected page token; invalidate it and schedule retry on `incompleteSearch=true`.
3. Download every unknown candidate and every current remote candidate for a pending logical transcript with at most four concurrent downloads.
4. Stream-cap and validate remote schema/canonical payload under Section 15.8 before it enters transcript storage.
5. Resolve each complete validated observed candidate set with the pure set function in Section 15.8 before deciding an outbound winner. A pending stale live mutation may never overwrite an observed tombstone merely because it was queued first or has a higher revision.
6. Persist candidates, incoming winners, and sync metadata transactionally.
7. Publish only a still-eligible resolved local winner not present in the accumulated validated observed set through the retry-safe generated-ID binding protocol in Section 15.4, with creates serialized per logical transcript. This observation does not prove global absence. An `unbound` operation that loses may be cancelled or superseded; a bound or ambiguous operation must retain and resolve its frozen attempted identity.
8. Union validated candidates from the initial pass, exact-ID readbacks, and post-create passes. After any create, perform the bounded stabilization protocol from Section 15.4 and resolve each accumulated set after every complete valid pass. Page races before, between, or after pages therefore add observed candidates without making absence authoritative.
9. If stabilization fails by the fourth complete pass, keep the operation pending, remain Syncing, schedule bounded-backoff reconciliation, and perform zero cleanup. Mark reconcile success time only after all non-permanent operations finish or are durably classified; mark an item Synced only when its exact local candidate is verified by known file ID, wins the stabilized observed set, and no pending operation remains. This status never claims a global snapshot.

Remote publication uses generated-ID immutable multipart create, same-ID retry/409 verification, exact-file readback, bounded stabilization, set resolution, and constrained permanent-delete cleanup from Section 15.4. Creates remain serialized per logical transcript even while downloads run at concurrency four.

### 15.8 Conflict order

Resolution is a pure function of the complete validated candidate set. It is independent of input enumeration, Drive pagination order, download completion order, and IndexedDB iteration order. Apply this exact algorithm:

1. Deduplicate byte-identical physical copies by validated `candidateHash`; choose the lowest lexicographic Drive file ID as representative.
2. Define the regular total order by this exact ascending comparison precedence, selecting the maximum:
   ...
   4. Accepted-payload hash lexicographic ASCII order.
3. Let `dominantTombstone` be the maximum tombstone under the regular order, if any.
4. If no tombstone exists, select the maximum live candidate under the regular order.
5. If `dominantTombstone` exists, a live candidate is restore-eligible only when `restoredFromDeletionId === dominantTombstone.deletionId` and the live revision is greater than the dominant tombstone revision.
6. If one or more restore-eligible live candidates exist, select their regular-order maximum; otherwise select `dominantTombstone`.
7. Preserve every other distinct candidate as a loser/conflict under the retention rules.

The accepted-payload tie hash is total for tombstones and lives. It is always the existing lowercase hexadecimal SHA-256 over the exact canonical accepted-payload projection in Section 15.4; for a tombstone that same projection has `transcript:null`. No second tombstone hash algorithm exists. After validated-candidate deduplication, equal regular-order inputs imply the same canonical envelope and `candidateHash`; physical duplicate tie semantics were already resolved by the lowest Drive file ID representative. Drive file ID never orders distinct candidates.

Only an explicit user Undo/Restore action that observed an exact tombstone may originate `restoredFromDeletionId`. Valid later mutations descended from that restored live record preserve the field; stale live mutations and merge/sync code cannot synthesize, substitute, or guess it. A later delete receives a fresh `deletionId`. For the cycle `T1(d1, rev5)`, live restore of `d1` at `rev6`, and `T2(d2, rev4)`, `T1` is the dominant tombstone and the live restore is eligible and selected; `T2` remains a loser. This result is invariant under every enumeration.

The dominant-tombstone set rule, not revision ordering alone, prevents resurrection. Revision precedes clocks so clock skew cannot let a lower revision win within regular ordering. Equal revisions are concurrent and use the remaining total tie-breakers. `updatedAt` does not independently establish causality. Every compared `updatedAt` is a bounded epoch-millisecond safe integer under Section 15.4. A legacy record with an invalid timestamp is noncanonicalizable and remains in `migrationQuarantine`; it never enters conflict ordering. Invalid remote timestamps fail parser validation. `modifiedTime` and Drive `version` never participate.
...
Only after a complete in-cap body has a full raw-byte digest may intake decode as strict UTF-8 and call `JSON.parse`. Accept only a non-array plain object matching the exact schema-2 envelope, payload, and segment tables in Section 15.4; reject every unknown or missing field at every level. Require `schemaVersion` to be the safe integer `2` and reject every other value rather than partially interpreting it. Before canonical whitespace handling, bound checks, persistence, key derivation, RFC 8785 serialization, `candidateHash`, or accepted-payload hashing, validate every parsed string as a Unicode scalar sequence. The explicit validator must reject malformed scalar input before the canonicalization library is invoked. JSON escapes such as a lone `\uD800`, lone `\uDC00`, or reversed low/high pair remain malformed after parsing and are rejected, while their complete raw bodies retain `bodyByteHash` with `bodyHashScope = "full"`; a valid escaped high/low pair is one scalar. Validate every JSON type, nullability rule, scalar and UTF-8 byte bound, enum, epoch/relative-millisecond unit and range, legal tombstone/live lineage combination, derived-text equality, normalized segment text, globally unique segment ID, and segment order invariant. Segment text must satisfy the post-normalization 1 MiB limit and final joined text the 16 MiB aggregate limit. A live payload containing envelope fields such as `id`, `transcriptId`, or `updatedAt`, or any other extension, is an unknown-field failure. Reject rather than repair remote input. The total 25 MiB stream cap remains authoritative in addition to all field bounds.

Persist every distinct validated incoming candidate, winner or loser, in the dedicated account-neutral `conflictCandidates` store before presenting, resolving, or applying it. Candidate data includes `candidateHash`, revision, updatedAt, deviceId, deletedAt, deletionId, restoredFromDeletionId, transcript/null, and accepted-payload hash; it excludes account identity and Drive identifiers. The Drive layer associates a candidate with account key, remoteKey, and Drive file metadata in `syncMetadata`; editor draft/candidate payloads remain account-neutral. Invalid records never enter `transcripts` or `conflictCandidates`. Local `conflictCandidates` may prune an older same-device live candidate only after a strictly higher-revision same-device live successor is verified and only when no draft, conflict UI, restore lineage, pending operation, or sync-verification record references it. Never prune tombstones, same-revision divergence, cross-device candidates, or unresolved user-visible conflicts. Store only scalar-valid bounded remote quarantine metadata: Drive file ID up to 255 scalar values, `remoteKey` when independently valid, stable error code up to 128 ASCII bytes, and the `bodyByteHash`/`bodyHashScope`/`sizeAtLeast` or fingerprint-unavailable disposition defined above. The raw-byte fingerprint is not a `remoteKey`, `candidateHash`, or RFC 8785 payload hash. Never store arbitrary body bytes, provider response text, or parsed transcript fragments.
...
| Failure | Behavior |
| --- | --- |
| Offline/network/408/429/5xx during bound publication | Retry with bounded exponential backoff and jitter using the same `generatedFileId`, `attemptedCandidateHash`, and exact `attemptedEnvelopeJson` body |
| 409 during bound create | GET metadata plus media for `generatedFileId`; verify exact body, full-envelope hash, accepted-payload hash, filename, MIME type, and appProperties; matching content is success, mismatch is Needs attention |
| 401/403 caused by auth or omitted required scope | Pause queue; localized Needs reconnect; reacquire only from explicit Connect, Reconnect, or Sync now gesture |
| Transient 404/unreadable same-ID response after ambiguous create | Retry boundedly with the same generated ID and exact body; after the existing retry budget is exhausted, mark Needs attention; never bind a second ID |
| 404 during cleanup | Relist all pages and resolve current candidates; never infer candidate loss from a stale mapping or list absence |
...
Backoff schedule uses base 1 second, doubles to maximum 60 seconds, adds 0-25% jitter, and stops automatic per-operation attempts after 7 failures. Bound publication retries never allocate another ID or change attempted bytes. New online/focus/manual triggers may resume unbound transient failures; a bound operation that exhausted its ambiguous same-ID verification budget remains Needs attention until explicit recovery. Auth failures do not consume destructive retries.

### 15.10 Tombstone retention

Remote tombstone candidates and local causal ordering identity are permanent for this redesign. After 180 days following a confirmed reconcile, and only when no known operation is pending, compaction may remove retry history and auxiliary error state from sync implementation metadata. It must retain transcript ID, remoteKey, candidateHash, revision, updatedAt, deletedAt, deletionId, `restoredFromDeletionId` null state, device ID, accepted-payload hash, account association/confirmation needed for ordering, and the complete remote tombstone JSON. Do not compact while signed out or auth-paused. Physical local or remote tombstone deletion is out of scope until a future replica-watermark protocol can prove every replica observed the deletion. Distinct cross-device losing candidates receive the same retention protection. Section 15.4 permits only verified identical-copy cleanup and strictly older same-device live-candidate cleanup.
...
| `conflictCandidates` | Candidate ID | Every distinct validated account-neutral incoming transcript/tombstone candidate, including full-envelope `candidateHash`, causal deletion lineage, and comparison fields; no account/Drive identifiers |
| `syncMetadata` | Account key + transcript ID | Derived remoteKey, candidate/file association, informational Drive version, confirmed candidate/order/lineage, item state, last error code |
| `pendingOperations` | Account key + transcript ID | Durable desired publication plus frozen bound attempt: exact `generatedFileId`, `attemptedCandidateHash`, `attemptedEnvelopeJson`, `desiredCandidateHash`, `desiredEnvelopeJson`, `publicationState`, existing retry fields, and exact verification metadata from Section 15.4 |
...
For `pendingOperations`, `publicationState` is exactly `unbound | bound | creating | verifying | needs-attention`. Coalescing changes only `desiredCandidateHash` and `desiredEnvelopeJson`. It may determine the attempted candidate while `unbound`; it never changes `generatedFileId`, `attemptedCandidateHash`, or `attemptedEnvelopeJson` after binding. Attempted and desired fields may differ durably while a same-ID ambiguous create is resolved.
...
- Transcript create/edit/rename: write transcript revision and coalesced latest pending candidate publication together.
...
- Publication binding: after `files.generateIds`, atomically persist `generatedFileId`, frozen `attemptedCandidateHash`, exact `attemptedEnvelopeJson`, candidateHash and accepted-payload hash, filename, MIME type, appProperties, and `publicationState = bound` before any network create. Transactionally set `publicationState = creating` before sending the same multipart body.
...
- Sync success: after exact same-ID readback/409 verification and stabilized-set resolution, update confirmed candidate metadata and delete the matching attempted publication together; when desired differs from attempted, atomically replace it with a new `unbound` publication carrying the latest `desiredCandidateHash` and `desiredEnvelopeJson`.
...
- **Checkpoint A — identity/transport:** approved and verified granted scopes, attempt IDs/watchdogs, optional UserInfo display fields and fallback-first avatar handling, exact-host CSP, 60-second token margin, expiry pause, gesture-only Connect/Reconnect/Sync-now reacquisition, GIS revoke plus blast-radius copy, and paginated remoteKey candidate discovery.
- **Checkpoint B — durable outbound:** account consent/association; account+transcript desired-candidate coalescing before binding; pre-create `files.generateIds`; transactional persistence of `generatedFileId`, frozen `attemptedCandidateHash`/`attemptedEnvelopeJson`, desired fields, hashes, request metadata, and `publicationState`; serialized immutable multipart creates carrying metadata `id`; exact same-ID/body retry; lost-success recovery; 409 metadata+media verification; newer desired revision during ambiguity; no second ID or duplicate physical growth; local-save-first path for every transcription mode; full-envelope `candidateHash`; tombstone/restore JSON; safe-ID upload gate; legacy Drive migration; exact-file readback; and mismatch Needs attention.
- **Checkpoint C — inbound reconciliation/conflicts:** four-download concurrency, non-snapshot complete pagination, invalid-token restart, `incompleteSearch` invalidation, union of initial/readback/post-create observations, at most four stabilization passes requiring two identical consecutive complete sets without an intervening local create, strict bounded envelope parser, pure set resolver with dominant tombstone, exhaustive permutation convergence, durable preservation of every distinct candidate, concurrent losing-live conflict/history, dirty-editor protection, account-switch preview confirmation, unstable-scan pending/Syncing behavior with zero cleanup, constrained positive-ID cleanup, and offline/auth recovery.
...
- Unicode-scalar validation before canonical whitespace handling/counting/persistence/key/serialization/hashing; proof that RFC 8785 canonicalization is never invoked on malformed scalar input; JSON-escaped lone high surrogate, lone low surrogate, reversed pair, and valid surrogate pairs at scalar and UTF-8 boundaries; immutable `CANONICAL_WS` membership with `U+FEFF` and `U+0085` included and `U+200B` excluded; mixed-run collapse; exact outer-only title handling with internal runs preserved; no NFC/NFKC; all-`CANONICAL_WS` title/language/model/ID rejection without changing accepted source/language/model/ID strings; runtime and legacy seconds-to-`startMs`/`endMs` rounding/range checks; deterministic local forward timing repair and strict remote timing rejection; synthesized and ordinary segment normalized text at exactly 1 MiB versus 1 MiB plus one byte; final joined text at exactly 16 MiB versus 16 MiB plus one byte; normalization cases that cross a byte boundary; split/merge/multiline paste/spanning-selection behavior in relative milliseconds; raw derivation; all-segment subtitle timing validation; invalid normalized-empty segment rejection; all-empty no-cue unavailability; millisecond subtitle formatting; and byte-identical canonical inputs for TXT/SRT/VTT/JSON/payload hash.
- Pure set resolver: deduplicate by validated `candidateHash` with lowest-file-ID representative; regular total order by revision/updatedAt/device/accepted-payload hash; dominant tombstone; exact matching greater-revision restore eligibility; preservation of every distinct loser; and independence from enumeration, pagination, download completion, and IndexedDB iteration. Exhaustively test all permutations for multiple deletion IDs, competing tombstones, matching/nonmatching restores, equal-order ties, and the cycle `T1(d1, rev5)`, live restore `d1` at `rev6`, `T2(d2, rev4)`. Pin tombstone tie hashing to the existing accepted-payload projection with `transcript:null`. Also cover safe next-revision overflow, same-device strictly older live cleanup/local pruning guards, same-revision/cross-device/tombstone retention, and permanent delete behavior.
- Retry-safe immutable publication: `files.generateIds` occurs before first create; one IndexedDB transaction persists generated ID, exact attempted body, both hashes, filename, MIME, appProperties, and state before the request; metadata `id` is supplied; transient/lost responses retry the same ID and exact body; 409 GET verifies metadata and media; mismatches become Needs attention; and no retry allocates a second ID. Cover `generatedFileId`, `attemptedCandidateHash`, `attemptedEnvelopeJson`, `desiredCandidateHash`, `desiredEnvelopeJson`, and every `publicationState` transition, including desired-candidate coalescing before binding, a newer desired revision during an ambiguous attempt, verified attempted success followed by a new desired publication, retry-budget exhaustion, and no duplicate physical growth.
- Non-transactional list stabilization: page-race fixtures insert candidates before, between, and after pages; invalid/rejected page tokens restart a pass; `incompleteSearch=true` invalidates it; each reconcile unions initial-pass, exact-ID-readback, and post-create candidates; stability requires two consecutive complete identical `(fileId,candidateHash)` sets without an intervening local create and stops after at most four passes. Unstable scans remain Syncing with a pending operation, schedule bounded backoff, perform zero cleanup, and never claim global absence or a snapshot.
...
- Schema/hash fixtures use the normative live example plus `U+FEFF`, `U+0085`, `U+200B`, mixed canonical-whitespace runs, valid surrogate pairs, and title internal-run preservation. They pin exact RFC 8785 bytes plus the lowercase digest for the deletion-lineage projection, and separately pin exact canonical full-envelope bytes plus the 43-character unpadded base64url `candidateHash`. They prove `transcriptId` occurs once in the envelope and never in accepted-payload hash input/payload; segment keys are exactly `startMs`/`endMs`; parsing, editor serialization, and canonicalizable legacy migration produce identical payload bytes; repeated hashing is stable; malformed Unicode prevents any canonicalizer call and yields no RFC 8785 digest even when remote quarantine has a raw `bodyByteHash`; and implementation SHA-256 equals an independent Web Crypto digest of fixed canonical bytes. A paired tombstone fixture pins explicit nulls, full-envelope candidate identity, and the tombstone projection.
- Backoff bounds/jitter range, auth pause, expired-token gesture requirement, omitted-scope rejection before UserInfo/Drive, transient/permanent classification.
...
- Identity menu, auth attempt-ID timeout/late callback rejection, GIS error callback, 60-second freshness margin, expiry requiring an explicit Connect/Reconnect/Sync-now gesture, no background `requestAccessToken`, omitted granted scopes, reload reconnect requirement, GIS revoke confirmed/unconfirmed states and same-Cloud-project blast-radius copy, scalar validation of UserInfo `sub`/`name`/`email`/`picture` before display/count/key/URL use, optional name/picture and display/initial/glyph fallbacks, avatar initial HTTPS/exact-host/no-credentials/standard-port validation, `redirect: "error"` fallback on any redirect, malformed Unicode, CORS/MIME/byte/timeout fallback without blocking identity/sync, and account-switch paused/preview/apply/disclosure confirmation behavior. Tests make no assertion about inspecting redirect hops or final `response.url`.
- Library visible item actions and responsive editor controls.
- Sync-state presentation for bound ambiguous publication, newer desired revision, same-ID verification, unstable scans, mismatch Needs attention, and transition to Synced only after stabilized winner verification and pending-operation removal.
- Drive sync integration with mocked IndexedDB/transport proves generated-ID binding commits before create dispatch, lost-success and 409 paths retain the same ID/body, a newer desired revision does not replace attempted state, mismatch exposes one Needs-attention recovery, and unstable scans expose Syncing without invoking cleanup.
...
8. `GIS-01`: sign-in dismissal/error/timeout/late callback; `GIS-02`: UserInfo stable sub identity, optional name/picture fallbacks, initial URL policy, `redirect: "error"`, and timeout/oversize/MIME/CORS fallback to Blob/initials/glyph without blocking sync; `GIS-03`: token freshness margin/expiry pauses reconciliation and requires an explicit Connect, Reconnect, or Sync now gesture, including after reload, with no background token request; `GIS-04`: omitted granted scopes block UserInfo/Drive as applicable, and GIS revoke confirmed/unconfirmed UI includes the same-Cloud-project blast-radius copy/link; `GIS-05`: different-account Cancel, Reconcile-without-upload preview without apply, explicit apply confirmation, and separate disclosure confirmation before upload.
9. `DRV-01`: two clients concurrently create distinct immutable candidates for one remoteKey; each uses generated-ID multipart create, exact readback, bounded stabilization, and deterministic set resolution so neither write is lost and every losing branch persists as conflict/history. `DRV-02`: full-envelope `candidateHash`, exact filename/three appProperties/property bounds, raw-ID absence, non-snapshot discovery pagination, page races before/between/after pages, invalid-token restart, incomplete-search retry, unknown-candidate 25 MiB intake, strict body/hash/name/metadata recomputation, and file-ID representative selection only for byte-identical duplicate copies. `DRV-03`: pure set resolution exhaustively permutes multiple deletion IDs, competing tombstones, matching/nonmatching restores, equal-order ties, and `T1(d1, rev5)`/live restore `d1 rev6`/`T2(d2, rev4)`; it proves dominant-tombstone and regular-order convergence, safe next revision, stabilized Synced eligibility, loser preservation, and no Drive metadata causality. `DRV-04`: raw response fingerprinting before decode/parse, full/prefix hash boundaries, canonicalizer-not-called malformed-scalar proof, exact accepted-payload and full-envelope hash fixtures, durable preservation of valid winners/losers, bounded invalid metadata, offline queue/retry/auth pause, four-download cap, and serialized per-transcript creates. `DRV-05`: strict legacy `{id}.json` migration verifies immutable candidate output before bounded permanent cleanup, uploads IDs only through 512 UTF-8 bytes, metadata-quarantines every invalid/oversize body including 513-byte IDs, retains invalid old files, and never truncates or partially imports. `DRV-06`: pre-bind desired coalescing; generated-ID and exact body persistence before request; lost-success response; same-ID 409 metadata+media verification; transient same-ID 404/unreadable retry; newer desired revision during ambiguity; verified attempt followed by new desired publication; mismatch Needs attention; no second ID or duplicate physical growth; four-pass/two-consecutive-set stabilization; unstable Syncing/pending with zero cleanup; and guarded local/remote cleanup preserving references, same-revision divergence, every tombstone, and distinct cross-device losers.
...
- [ ] Every schema-2, UserInfo/avatar-derived, and legacy string is scalar-validated before canonical whitespace handling, bounds, persistence, key derivation, RFC 8785 serialization, `candidateHash`, or payload hashing; lone/reversed/unpaired surrogates are rejected without replacement, valid pairs count once at scalar/UTF-8 boundaries, and instrumentation proves malformed scalar input never reaches the canonicalization library. Raw `bodyByteHash` alone runs before decoding and remains available for malformed bodies.
...
- [ ] Access/expiry remain memory-only; `GIS-03` pauses reconciliation inside the 60-second freshness margin and reacquires only from an explicit Connect, Reconnect, or Sync now gesture. `prompt: ''` is allowed only within that gesture; expiry and reload never trigger background token requests.
- [ ] Account key equals normalized Google issuer plus UserInfo `sub`; email is never used as key. Avatar validates only the initial HTTPS/exact-host/no-credentials/standard-port URL, fetches with `redirect: "error"`, and falls back to initials/generic glyph on any redirect, timeout, oversize body, non-image MIME, CORS prevention, or other fetch failure without blocking identity or sync. No acceptance check assumes browser access to redirect hops or final `response.url`.
- [ ] Granted scopes are verified before UserInfo/Drive calls. Revoke uses `google.accounts.oauth2.revoke`; success appears only after confirmed callback, absent/failing token reports unconfirmed revocation and links Google Account permissions, and confirmation names all Google authorizations granted to the same Cloud project.
...
- [ ] `DRV-01` and `DRV-02` prove concurrent immutable candidate create/stabilize/no-lost-write behavior; full-envelope `candidateHash`; exact candidate filename, MIME type, and three bounded appProperties; scalar validation before canonicalization/key derivation; non-snapshot complete pagination; invalid-token restart; incomplete-search invalidation; page races before/between/after pages; strict recomputation; durable losing branches; race-tolerant selection; and lowest-file-ID representative selection only for byte-identical physical duplicates.
- [ ] `DRV-03` proves the resolver is a pure set function independent of enumeration, pagination, download completion, and IndexedDB order. Exhaustive permutations cover multiple deletion IDs, competing tombstones, matching/nonmatching restores, equal-order ties, and `T1(d1, rev5)`/live restore `d1 rev6`/`T2(d2, rev4)`; deduplication, dominant tombstone, exact greater-revision restore eligibility, regular total order, tombstone `transcript:null` accepted-payload tie hash, loser retention, safe next revision, and permanent lineage-bearing tombstone JSON converge exactly.
- [ ] `DRV-04` proves incremental `bodyByteHash` runs before decode/parse/scalar validation; complete bodies through 25 MiB receive `full`; byte 25 MiB + 1 is not retained and yields exactly the first-25-MiB digest, `prefix-25MiB`, and `sizeAtLeast = 26,214,401`; only streaming/digest infrastructure failure omits the hash and it cannot permit parsing/application. It also proves raw fingerprints never affect identity, `remoteKey`, `candidateHash`, accepted-payload hashing, or conflict ordering; malformed escaped high/low/reversed-surrogate bodies retain full raw fingerprints but never invoke RFC 8785; and every exact Section 15.4 JSON type, required/nullability rule, pinned whitespace rule, scalar/UTF-8 and post-normalization size bound, valid-pair boundary, enum, epoch/relative unit and range, seven-day boundary, segment invariant, canonical derivation, envelope-only transcriptId, no-unknown-field rule, legal lineage combination, exact stable projection/full-envelope RFC 8785 bytes and hashes, durable account-neutral valid candidates, bounded invalid metadata, four-download maximum, and per-transcript create serialization passes.
- [ ] `DRV-05` proves strict bounded legacy Drive JSON import, exact field/timing/ID rejection, derived remoteKey/candidateHash publication and verification before old-file cleanup, 512-byte upload acceptance, metadata-only quarantine at 513 bytes and all larger/invalid bounds, invalid old-file retention, no partial import, and no truncation.
- [ ] UI states Local only, Pending, Syncing, Synced, Needs attention, last sync, and pending count equal repository/sync-service fixture state. Synced requires known-file-ID verification, winner status in a stabilized observed set, and no pending operation; it never claims a global snapshot.
- [ ] No raw Drive file ID appears.
- [ ] No source media or settings are uploaded.
- [ ] `DRV-06` proves `files.generateIds` precedes first create; one IndexedDB transaction persists generated ID, exact full-envelope body, both hashes, filename, MIME, appProperties, and attempt state before request; metadata `id` is supplied; lost success and every transient retry reuse the same ID/body; 409 GET verifies metadata+media; newer desired revision cannot rebind an ambiguous attempt; verified attempted success spawns a new latest-desired publication when needed; mismatch or exhausted same-ID verification becomes Needs attention; and physical file count does not grow from retries.
- [ ] Every pending publication uses exact fields `generatedFileId`, `attemptedCandidateHash`, `attemptedEnvelopeJson`, `desiredCandidateHash`, `desiredEnvelopeJson`, and `publicationState`, with only `unbound | bound | creating | verifying | needs-attention`; existing account/transcript/retry data remains durable.
- [ ] Every post-publication reconcile unions initial-pass, exact-ID-readback, and post-create candidates. Stability requires two consecutive complete identical `(fileId,candidateHash)` sets without an intervening local create and at most four complete passes. Failure remains Syncing/pending, schedules bounded backoff, and performs zero cleanup.
- [ ] Cleanup uses only positively identified, previously downloaded and validated file IDs. Absence never authorizes deletion. Remote cleanup and optional local `conflictCandidates` pruning retain tombstones, same-revision divergence, cross-device candidates, unresolved visible conflicts, and every candidate referenced by draft/UI/restore/pending/sync-verification state.
- [ ] Revoke copy states that existing backup remains and revocation disconnects every Google authorization granted to the same Cloud project.
...
| Lost create response or transient retry duplicates immutable Drive content | Duplicate physical growth or ambiguous publication | Preallocate with `files.generateIds`; transactionally freeze ID/body/hashes/metadata before create; retry only same ID/body; verify same-ID 409 by metadata+media GET; mismatch or exhausted ambiguity becomes Needs attention |
| Concurrent immutable Drive creates produce multiple candidates or duplicate copies | Ambiguous remote state or lost branch | Non-snapshot complete passes, unioned observations, two-consecutive-set stability within the four-pass cap, pure set resolution, durable candidate set, exact readback, and constrained positive-ID cleanup |
| Device clocks skew | Older state could appear newer | Pure dominant-tombstone set resolver; regular total order uses revision then bounded epoch milliseconds/device/hash; do not treat timestamps as causal clocks |
| Drive pagination changes during listing | False absence, premature Synced state, or unsafe cleanup | Treat listing as non-snapshot; restart invalid tokens; reject incomplete searches; require two consecutive identical complete observed sets within four passes; unstable scans stay pending/Syncing with zero cleanup |
| Token expiry, popup behavior, or omitted granular-consent scopes interrupt sync | Hidden auth failure or unauthorized calls | Attempt IDs, GIS error callback, bounded watchdog, granted-scope verification, 60-second margin, reconciliation pause, and gesture-only Connect/Reconnect/Sync-now reacquisition |
| Revoke cannot be confirmed or its project-wide effect is misunderstood | UI overstates removed access or understates impact | Use GIS revoke callback, clear local session, label unconfirmed outcomes, link Google Account permissions, and name every authorization under the same Cloud project |
...
| Raw-body, identity-key, full-envelope candidate, and accepted-payload digests become conflated | Invalid input affects identity/order or malformed bodies lose forensic correlation | Keep `bodyByteHash`, `remoteKey`, `candidateHash`, and RFC 8785 accepted-payload hash separate by name, input, encoding, timing, storage, and permitted use; raw/full-envelope digests never enter conflict ordering |
| JavaScript or an RFC 8785 package accepts/repairs unpaired UTF-16 surrogates | Different keys/hashes, corrupted display, or cross-runtime disagreement | Explicit surrogate-pair validator before canonical whitespace handling, byte/scalar bounds, persistence, remoteKey, RFC 8785, and all canonical hashes; prove canonicalizer is never called for malformed input; hard rejection without `U+FFFD` |
| Stale live record has a high revision | Deleted transcript resurrects | Select dominant tombstone across the full set; only matching-deletion, greater-revision lives are restore-eligible; preserve later/different deletion IDs as conflicts |
...
| Append-only tombstones and concurrent losing branches grow storage | Long-term metadata accumulation | Coalesce desired candidates only before binding; delete verified identical copies and strictly older same-device live revisions; locally prune only unreferenced verified successors; compact retry/error auxiliaries after confirmed 180-day reconcile; retain tombstones, same-revision divergence, cross-device branches, and visible conflicts until future replica-watermark protocol |
...
21. GIS access tokens/expiry remain memory-only and its browser token model has no refresh token. The 60-second freshness margin pauses reconciliation. Token reacquisition requires an explicit Connect, Reconnect, or Sync now gesture; `prompt: ''` is permitted only during that gesture. Granted scopes are verified before protected calls.
22. Identity comes from bounded OIDC UserInfo; name/picture are optional, verified email is display-only, and account key is normalized issuer plus `sub`. Avatar is fallback-first: validate the initial exact HTTPS host/no-credentials/standard-port URL, use a bounded `redirect: "error"` fetch, and fall back locally without blocking identity/sync because picture URL/host/CORS stability is not guaranteed. Browser redirect hops/final URL are not inspected. Revoke uses `google.accounts.oauth2.revoke`, states its same-Cloud-project authorization blast radius, is confirmed only by callback, and never erases backup.
23. Conflict resolution is a pure set function. Deduplicate byte-identical copies by validated `candidateHash` with lowest Drive file ID as representative; define regular total order by revision, bounded epoch-millisecond `updatedAt`, ASCII device ID, then the existing lowercase RFC 8785/SHA-256 accepted-payload hash; choose the regular-order maximum tombstone as `dominantTombstone`; then select the maximum live only when it restores that exact deletion ID at a greater revision, otherwise select the dominant tombstone. With no tombstone, select the maximum live. Preserve every distinct loser. Tombstone tie hashing uses the existing projection with `transcript:null`. Results never depend on enumeration, pagination, download completion, or IndexedDB order.
...
33. Drive uses immutable append-only schema-2 candidates. Scalar validation and full-envelope RFC 8785 serialization produce a separate 43-character unpadded base64url `candidateHash`; exact filename is `whisdom-transcript-<remoteKey>-<candidateHash>.json` with the three pinned private properties. Before first create, publication obtains one appDataFolder ID from `files.generateIds`, durably freezes that ID and exact body/hashes/metadata, supplies metadata `id`, and retries only the same ID/body. Same-ID 409 metadata+media verification can establish success; mismatch or exhausted ambiguity becomes Needs attention. Drive time/version metadata never establishes causality.
...
41. Pending Drive publications coalesce by account plus transcript only while `publicationState` is `unbound`. Exact fields are `generatedFileId`, `attemptedCandidateHash`, `attemptedEnvelopeJson`, `desiredCandidateHash`, `desiredEnvelopeJson`, and `publicationState` with `unbound | bound | creating | verifying | needs-attention`; existing account/transcript/retry fields remain. After binding, attempted ID/hash/body are immutable, newer local state updates only desired fields, and verified attempted success creates a new unbound latest-desired publication when different. No ambiguous attempt is automatically rebound.
42. Drive listing is non-snapshot and absence is never authoritative. Reconcile unions initial-pass, exact-ID-readback, and post-create validated observations. After publication it runs at most four complete stabilization passes and requires two consecutive identical `(fileId,candidateHash)` sets with no intervening local create. Unstable scans remain pending/Syncing, schedule bounded backoff, and perform no cleanup. Synced means known-file-ID verification, winner status in the stabilized observed set, and no pending operation—not a global snapshot.
43. Cleanup acts only on positively identified, previously downloaded and validated file IDs. Optional local conflict pruning requires a verified strictly higher-revision same-device live successor and no draft, conflict UI, restore lineage, pending operation, or sync-verification reference; tombstones, same-revision divergence, cross-device candidates, and unresolved visible conflicts remain.

## 27. Authoritative references

Technical Drive and GIS contracts above follow these official references, accessed 2026-07-29:

- Drive API v2-to-v3 reference: https://developers.google.com/workspace/drive/api/guides/v2-to-v3-reference
