...
export type CompatibilitySettingsProjection = Omit<
  AppSettings,
  "explicitModelId"
> & {
...
function ok<T>(value: T): CompatibilityResult<T> {
  return { ok: true, value }
}
function fail<T>(
  error: CompatibilityFailureCode,
  path: string
...
function hasNonWhitespace(value: string): boolean {
  return [...value].some((part) => !isCanonicalWhitespace(part.codePointAt(0)!))
}
function boundedString(
  ...
  if (typeof value !== "string")
    return fail("compatibility.invalid-shape", path)
  ...
  if (
    ...
    (requireVisible && !hasNonWhitespace(value))
  )
    return fail("compatibility.out-of-bounds", path)
  return ok(value)
}
function canonicalId(
  ...
  if (
    ...
    !new RegExp(`^${prefix}[A-Za-z0-9_-]{22}$`).test(value)
  )
    return fail("compatibility.invalid-scalar", path)
  try {
    const suffix = value.slice(2)
    const bytes = Uint8Array.from(
      atob(suffix.replace(/-/g, "+").replace(/_/g, "/") + "=="),
    ...
    const encoded = btoa(String.fromCharCode(...bytes))
      ...
      .replace(/\//g, "_")
      .replace(/=+$/u, "")
    return bytes.length === 16 && encoded === suffix
      ? ok(value)
      : fail("compatibility.invalid-scalar", path)
  } catch {
    return fail("compatibility.invalid-scalar", path)
  }
}

export async function inspectVersion2Schema(
  db: IDBPDatabase
): Promise<CompatibilityResult<Version2SchemaDescription>> {
  const stores = Array.from(db.objectStoreNames).sort()
  if (
    stores.length !== SCHEMA.stores.length ||
    stores.some((store, index) => store !== SCHEMA.stores[index])
  )
    return fail("compatibility.invalid-schema", "$.stores")
  const transaction = db.transaction([...SCHEMA.stores], "readonly")
  for (const storeName of SCHEMA.stores) {
    const store = transaction.objectStore(storeName)
    if (!sameKeyPath(store.keyPath, SCHEMA.keyPaths[storeName]))
      return fail("compatibility.invalid-schema", `$.keyPaths.${storeName}`)
    const indexes = Array.from(store.indexNames).sort()
    const expected = [...SCHEMA.indexes[storeName]]
    if (
      indexes.length !== expected.length ||
      indexes.some((name, index) => name !== expected[index])
    )
      return fail("compatibility.invalid-schema", `$.indexes.${storeName}`)
  }
  const deviceId = parseCompatibilityDeviceId(
    await transaction.objectStore("meta").get("deviceId")
  )
  if (!deviceId.ok)
    return fail("compatibility.invalid-schema", "$.meta.deviceId")
  await transaction.done
  return ok({ ...SCHEMA, deviceId: deviceId.value })
}

export function parseCompatibilityDeviceId(
  value: unknown
): CompatibilityResult<string> {
  return canonicalId(value, "d_", "$")
}
export function parseCompatibilityDeletionId(
  value: unknown
): CompatibilityResult<string> {
  return canonicalId(value, "x_", "$")
}
export function parseCompatibilityEpochMs(
  value: unknown
): CompatibilityResult<number> {
  if (typeof value !== "number") return fail("compatibility.invalid-shape", "$")
  return Number.isSafeInteger(value) &&
    value >= MIN_EPOCH_MS &&
    value <= MAX_EPOCH_MS
    ? ok(value)
    : fail("compatibility.out-of-bounds", "$")
}
export function parseCompatibilityLegacyIso(
  value: unknown
): CompatibilityResult<number> {
  ...
  if (!Number.isFinite(epoch) || new Date(epoch).toISOString() !== value)
    return fail("compatibility.invalid-scalar", "$")
  const bounded = parseCompatibilityEpochMs(epoch)
  return bounded.ok ? bounded : fail(bounded.error, "$")
}
export function legacySecondsToCompatibilityMs(
  value: unknown
): CompatibilityResult<number> {
  if (typeof value !== "number" || !Number.isFinite(value) || value < 0)
    return fail("compatibility.time-conversion", "$")
  const product = value * 1000
  const rounded = Math.round(product)
  return Number.isFinite(product) &&
    Number.isSafeInteger(rounded) &&
    rounded <= MAX_RELATIVE_MS
    ? ok(rounded)
    : fail("compatibility.time-conversion", "$")
}
export function compatibilityMsToLegacySeconds(
  value: unknown
): CompatibilityResult<number> {
  if (
    typeof value !== "number" ||
    ...
    value < 0 ||
    value > MAX_RELATIVE_MS
  )
    return fail("compatibility.time-conversion", "$")
  const seconds = value / 1000
  return Math.round(seconds * 1000) === value
    ? ok(seconds)
    : fail("compatibility.time-conversion", "$")
}
...
function trimCanonicalOuterWhitespace(value: string): string {
  ...
  while (start < end && isCanonicalWhitespace(scalars[start].codePointAt(0)!))
    start += 1
  while (end > start && isCanonicalWhitespace(scalars[end - 1].codePointAt(0)!))
    end -= 1
...
function relativeMs(value: unknown, path: string): CompatibilityResult<number> {
  if (typeof value !== "number")
    return fail("compatibility.invalid-shape", path)
  return Number.isSafeInteger(value) && value >= 0 && value <= MAX_RELATIVE_MS
    ? ok(value)
    : fail("compatibility.out-of-bounds", path)
}
function parseSegment(
  value: unknown,
  path: string
): CompatibilityResult<RollbackV2Segment> {
  if (!isPlainObject(value) || !hasExactKeys(value, SEGMENT_KEYS))
    return fail("compatibility.invalid-shape", path)
  const id = boundedString(value.id, `${path}.id`, 1, 255, 1024, true)
  if (!id.ok) return id
  const startMs = relativeMs(value.startMs, `${path}.startMs`)
  if (!startMs.ok) return startMs
  const endMs = relativeMs(value.endMs, `${path}.endMs`)
  if (!endMs.ok) return endMs
  if (endMs.value < startMs.value)
    return fail("compatibility.invalid-lineage", `${path}.endMs`)
  if (typeof value.text !== "string")
    return fail("compatibility.invalid-shape", `${path}.text`)
  if (!isScalarString(value.text))
    return fail("compatibility.invalid-scalar", `${path}.text`)
  if (normalizeSegmentText(value.text) !== value.text)
    return fail("compatibility.invalid-derived-text", `${path}.text`)
  if (byteLength(value.text) > 1024 * 1024)
    return fail("compatibility.out-of-bounds", `${path}.text`)
  return ok({
    id: id.value,
...
export function parseRollbackV2Segment(
  value: unknown
): CompatibilityResult<RollbackV2Segment> {
...
function parsePayload(
  value: unknown,
  path: string
): CompatibilityResult<RollbackV2Payload> {
  if (!isPlainObject(value) || !hasExactKeys(value, PAYLOAD_KEYS))
    return fail("compatibility.invalid-shape", path)
  const title = boundedString(value.title, `${path}.title`, 1, 512, 2048, true)
  if (!title.ok) return title
  if (trimCanonicalOuterWhitespace(title.value) !== title.value)
    return fail("compatibility.invalid-scalar", `${path}.title`)
  ...
  if (typeof value.mode !== "string")
    return fail("compatibility.invalid-shape", `${path}.mode`)
  if (
    !isScalarString(value.mode) ||
    !MODES.includes(value.mode as ProcessingMode)
  )
    return fail("compatibility.invalid-scalar", `${path}.mode`)
  const createdAt = parseCompatibilityEpochMs(value.createdAt)
  if (!createdAt.ok) return fail(createdAt.error, `${path}.createdAt`)
  if (typeof value.text !== "string")
    return fail("compatibility.invalid-shape", `${path}.text`)
  if (!isScalarString(value.text))
    return fail("compatibility.invalid-scalar", `${path}.text`)
  if (byteLength(value.text) > 16 * 1024 * 1024)
    return fail("compatibility.out-of-bounds", `${path}.text`)
  if (!Array.isArray(value.segments))
    return fail("compatibility.invalid-shape", `${path}.segments`)
  if (value.segments.length < 1 || value.segments.length > 100000)
    return fail("compatibility.out-of-bounds", `${path}.segments`)
  const segments: RollbackV2Segment[] = []
  ...
  for (let index = 0; index < value.segments.length; index += 1) {
    ...
    if (
      ids.has(segment.value.id) ||
      (index > 0 && segment.value.startMs < previousEnd)
    )
      return fail("compatibility.invalid-lineage", `${path}.segments[${index}]`)
    ids.add(segment.value.id)
    previousEnd = segment.value.endMs
  ...
  const derived = segments
    ...
    .filter(Boolean)
    .join(" ")
  if (byteLength(derived) > 16 * 1024 * 1024)
    return fail("compatibility.out-of-bounds", `${path}.text`)
  if (value.text !== derived)
    return fail("compatibility.invalid-derived-text", `${path}.text`)
  return ok({
    title: title.value,
...
export function parseRollbackV2Payload(
  value: unknown
): CompatibilityResult<RollbackV2Payload> {
...
export function parseRollbackV2Envelope(
  value: unknown
): CompatibilityResult<RollbackV2Envelope> {
  if (!isPlainObject(value) || !hasExactKeys(value, ENVELOPE_KEYS))
    return fail("compatibility.invalid-shape", "$")
  if (typeof value.schemaVersion !== "number")
    return fail("compatibility.invalid-shape", "$.schemaVersion")
  if (!Number.isSafeInteger(value.schemaVersion) || value.schemaVersion !== 2)
    return fail("compatibility.out-of-bounds", "$.schemaVersion")
  const transcriptId = boundedString(
    value.transcriptId,
    "$.transcriptId",
    1,
    512,
  ...
  if (!transcriptId.ok) return transcriptId
  if (typeof value.revision !== "number")
    return fail("compatibility.invalid-shape", "$.revision")
  if (!Number.isSafeInteger(value.revision) || value.revision < 0)
    return fail("compatibility.out-of-bounds", "$.revision")
  const updatedAt = parseCompatibilityEpochMs(value.updatedAt)
  if (!updatedAt.ok) return fail(updatedAt.error, "$.updatedAt")
  const deviceId = canonicalId(value.deviceId, "d_", "$.deviceId")
  if (!deviceId.ok) return deviceId
  const deletedAt =
    value.deletedAt === null
      ? ok<number | null>(null)
      : parseCompatibilityEpochMs(value.deletedAt)
  if (!deletedAt.ok) return fail(deletedAt.error, "$.deletedAt")
  const deletionId =
    value.deletionId === null
      ? ok<string | null>(null)
      : canonicalId(value.deletionId, "x_", "$.deletionId")
  if (!deletionId.ok) return deletionId
  const restored =
    value.restoredFromDeletionId === null
      ? ok<string | null>(null)
      : canonicalId(
          value.restoredFromDeletionId,
          "x_",
          "$.restoredFromDeletionId"
        )
  ...
  const transcript =
    value.transcript === null
      ? ok<RollbackV2Payload | null>(null)
      : parsePayload(value.transcript, "$.transcript")
  ...
  return ok({
    ...
    restoredFromDeletionId: restored.value,
...
export function projectRollbackEnvelope(
  ...
  const text = parsed.value.transcript.segments
    .map((segment) => segment.text)
    .filter(Boolean)
    .join(" ")
  return ok({
    id: parsed.value.transcriptId,
    title: parsed.value.transcript.title,
    sourceName: parsed.value.transcript.sourceName,
    language: parsed.value.transcript.language,
    modelId: parsed.value.transcript.modelId,
    mode: parsed.value.transcript.mode,
    createdAt: new Date(parsed.value.transcript.createdAt).toISOString(),
    updatedAt: new Date(parsed.value.updatedAt).toISOString(),
    text,
    segments,
  })
}
...
export function parseCompatibilitySettings(
  ...
  if (
    !isScalarString(value.uiLanguage) ||
    (value.uiLanguage !== "en" && value.uiLanguage !== "vi")
  )
    return fail("compatibility.invalid-scalar", "$.uiLanguage")
  const modelId = boundedString(value.modelId, "$.modelId", 1, 128, 512, true)
  if (!modelId.ok) return modelId
  const language = boundedString(
    value.language,
    "$.language",
  ...
  if (!language.ok) return language
  if (typeof value.mode !== "string")
    return fail("compatibility.invalid-shape", "$.mode")
  if (
    !isScalarString(value.mode) ||
    !MODES.includes(value.mode as ProcessingMode)
  )
    return fail("compatibility.invalid-scalar", "$.mode")
  if (
    typeof value.chunkSeconds !== "number" ||
    !Number.isFinite(value.chunkSeconds) ||
    value.chunkSeconds <= 0
  )
    return fail("compatibility.out-of-bounds", "$.chunkSeconds")
  if (
    typeof value.overlapSeconds !== "number" ||
    !Number.isFinite(value.overlapSeconds) ||
  ...
  return ok({
    ...
    modelId: modelId.value,
    explicitModelId: explicitModelId.value,
    language: language.value,
    mode: value.mode as ProcessingMode,
