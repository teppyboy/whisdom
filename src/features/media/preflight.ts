...
export async function analyzeMediaFile(
  ...
  const memoryRisk =
    fileSizeMb > 1500 ? "high" : fileSizeMb > 500 ? "medium" : "low"
...
function recommendMode(
  ...
  if (requestedMode === "server" || requestedMode === "local-helper") {
...
export async function getWebGpuStatus(): Promise<WebGpuStatus> {
  ...
  if (typeof window !== "undefined" && !window.isSecureContext) {
    return {
      available: false,
      reason:
        "WebGPU requires HTTPS or localhost; this page is not a secure context",
    }
  }
